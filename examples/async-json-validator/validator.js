...
async function validate_file(path) {
  ...
  const __match_val_48 = await fs.readFile(path);
  switch (__match_val_48.$tag) {
    case "Ok": {
      const content = __match_val_48.$payload;
      const text = content;
      const __match_val_49 = parse(text);
      switch (__match_val_49.$tag) {
        ...
        case "Err": {
          const error = __match_val_49.$payload;
    ...
    case "Err": {
      const error = __match_val_48.$payload;
      const message = error;
...
function format_error(error) {
  const __match_val_50 = error;
  switch (__match_val_50.$tag) {
    case "UnexpectedToken": {
      const tok = __match_val_50.$payload[0];
      const pos = __match_val_50.$payload[1];
    ...
    case "InvalidNumber": {
      const s = __match_val_50.$payload;
    ...
    case "UnexpectedChar": {
      const c = __match_val_50.$payload[0];
      const pos = __match_val_50.$payload[1];
