...
var registerDevtoolsSignal = /* @__PURE__ */ __name((signal) => {
  devtoolsSignals.add(signal);
}, "registerDevtoolsSignal");
var unregisterDevtoolsSignal = /* @__PURE__ */ __name((signal) => {
...
var _Signal = class _Signal {
  constructor(initial) {
    ...
    __publicField(this, "__luminaDevtoolsId", nextDevtoolsSignalId++);
    __publicField(this, "value");
    this.value = __lumina_clone(initial);
    registerDevtoolsSignal(this);
  }
  get() {
...
var nextDevtoolsSignalId = 1;
var nextDevtoolsRootId = 1;
var devtoolsSignals = /* @__PURE__ */ new Set();
var devtoolsRoots = /* @__PURE__ */ new Map();
var devtoolsListeners = /* @__PURE__ */ new Set();
var devtoolsNotifyPending = false;
...
var createStaticSignal = /* @__PURE__ */ __name((value) => {
  ...
  return {
    ...
    get: /* @__PURE__ */ __name(() => __lumina_clone(current), "get"),
    peek: /* @__PURE__ */ __name(() => __lumina_clone(current), "peek"),
...
var findSingleMove = /* @__PURE__ */ __name((previous, next, equals) => {
...
var analyzeSequenceTransition = /* @__PURE__ */ __name((previous, next, equals) => {
  ...
  if (previous.length >= 2) {
    ...
    const right = left + 1;
    if (left >= 0 && right < previous.length && equals(previous[left], next[right]) && equals(previous[right], next[left])) {
...
var analyzeKeyedChildTransition = /* @__PURE__ */ __name((prevChildren, nextChildren) => {
  if (prevChildren.length !== nextChildren.length || !prevChildren.every((child) => hasVNodeKey(child)) || !nextChildren.every((child) => hasVNodeKey(child))) {
    return null;
  }
  const assertUniqueKeys = /* @__PURE__ */ __name((children2) => {
    const seenKeys = /* @__PURE__ */ new Set();
    for (const child of children2) {
  ...
  return analyzeSequenceTransition(prevChildren, nextChildren, (left, right) => hasVNodeKey(left) && hasVNodeKey(right) ? left.key === right.key : false);
...
var canSkipDomPatch = /* @__PURE__ */ __name((prevNode, nextNode) => {
...
var patchPortalMount = /* @__PURE__ */ __name((anchor, prevNode, nextNode, documentLike, eventStore, portalStore, liveTextStore) => {
...
var bindIndexListHost = /* @__PURE__ */ __name((host, node, documentLike, eventStore, portalStore, liveTextStore) => {
  const source = node.itemsSignal;
  const renderItem = node.listRender;
  if (!source || typeof renderItem !== "function") {
  ...
  const renderChildren = /* @__PURE__ */ __name(() => itemSignals.map((itemSignal, index) => createDomNode(coerceRenderableToVNode(renderItem(itemSignal, index)), documentLike, eventStore, portalStore, liveTextStore)), "renderChildren");
...
var bindForListHost = /* @__PURE__ */ __name((host, node, documentLike, eventStore, portalStore, liveTextStore) => {
  const source = node.itemsSignal;
  const keyOf = node.listKey;
  const renderItem = node.listIndexedRender;
  if (!source || typeof keyOf !== "function" || typeof renderItem !== "function") {
  ...
  const createEntry = /* @__PURE__ */ __name((value, index) => {
    ...
    const domNode = createDomNode(applyVNodeKey(coerceRenderableToVNode(renderItem(itemSignal, indexSignal)), key), documentLike, eventStore, portalStore, liveTextStore);
    return {
      key,
      currentValue: value,
      currentIndex: index,
  ...
  host.__luminaForListEffect = new Effect(() => {
    const nextItems = readIndexListValues(source, true);
    if (nextItems.length === state2.order.length) {
      let sameOrder = true;
      for (let index = 0; index < nextItems.length; index += 1) {
        const key = coerceListKey(keyOf(nextItems[index], index), index);
        if (state2.order[index] !== key) {
          sameOrder = false;
          break;
        }
      }
    ...
    const transition = analyzeSequenceTransition(state2.order, nextOrder, (left, right) => left === right);
    ...
    if (transition.kind === "adjacent_swap" || transition.kind === "single_move") {
      const nextEntries2 = nextOrder.map((key) => {
        const entry = state2.entriesByKey.get(key);
        if (!entry) {
      ...
      state2.order = nextOrder;
      reorderChildren(host, nextEntries2.map((entry) => entry.domNode), eventStore, portalStore, liveTextStore, {
        transition,
...
var createDomNode = /* @__PURE__ */ __name((node, documentLike, eventStore, portalStore, liveTextStore) => {
  if (node.kind === "text") {
  ...
  if (node.kind === "fragment") {
    ...
    const children3 = asDomChildren(node).map((child) => createDomNode(child, documentLike, eventStore, portalStore, liveTextStore));
  ...
  const children2 = asDomChildren(node).map((child) => createDomNode(child, documentLike, eventStore, portalStore, liveTextStore));
  setChildren(element, children2);
  return element;
}, "createDomNode");
var patchDomChildrenPositionally = /* @__PURE__ */ __name((element, prevChildren, nextChildren, documentLike, eventStore, portalStore, liveTextStore) => {
...
var patchDomChildrenWithKeys = /* @__PURE__ */ __name((element, prevChildren, nextChildren, documentLike, eventStore, portalStore, liveTextStore) => {
  ...
  for (const nextChild of nextChildren) {
    if (hasVNodeKey(nextChild)) {
      ...
      nextDomChildren.push(canSkipDomPatch(prevEntry2.vnode, nextChild) ? prevEntry2.domNode : patchDomNode(prevEntry2.domNode, prevEntry2.vnode, nextChild, documentLike, eventStore, portalStore, liveTextStore));
      continue;
    }
    ...
    nextDomChildren.push(canSkipDomPatch(prevEntry.vnode, nextChild) ? prevEntry.domNode : patchDomNode(prevEntry.domNode, prevEntry.vnode, nextChild, documentLike, eventStore, portalStore, liveTextStore));
  }
...
var patchDomNode = /* @__PURE__ */ __name((domNode, prevNode, nextNode, documentLike, eventStore, portalStore, liveTextStore) => {
  ...
  if (nextNode.kind === "index_list") {
    ...
    bindIndexListHost(domNode, nextNode, documentLike, eventStore, portalStore, liveTextStore);
    return domNode;
  }
  if (nextNode.kind === "for_list") {
    updateDomProperties(domNode, prevNode.props, forListHostProps, eventStore);
    bindForListHost(domNode, nextNode, documentLike, eventStore, portalStore, liveTextStore);
    return domNode;
  }
  if (nextNode.kind === "portal") {
    patchPortalMount(domNode, prevNode, nextNode, documentLike, eventStore, portalStore, liveTextStore);
    return domNode;
  }
...
var hydrateDomNode = /* @__PURE__ */ __name((domNode, node, documentLike, eventStore, portalStore, liveTextStore) => {
  if (node.kind === "text") {
  ...
  if (node.kind === "index_list") {
    ...
    bindIndexListHost(domNode, node, documentLike, eventStore, portalStore, liveTextStore);
    return domNode;
  }
  if (node.kind === "for_list") {
    updateDomProperties(domNode, void 0, forListHostProps, eventStore);
    bindForListHost(domNode, node, documentLike, eventStore, portalStore, liveTextStore);
    return domNode;
  }
  if (node.kind === "portal") {
    patchPortalMount(domNode, vnodePortal(node.target, []), node, documentLike, eventStore, portalStore, liveTextStore);
    return domNode;
  }
  const element = domNode;
  ...
  for (let index = 0; index < nextChildren.length; index += 1) {
    ...
    nextDomChildren.push(currentChild ? hydrateDomNode(currentChild, nextChild, documentLike, eventStore, portalStore, liveTextStore) : createDomNode(nextChild, documentLike, eventStore, portalStore, liveTextStore));
  }
...
var renderAppVNode = /* @__PURE__ */ __name((componentFn, props) => {
  const frameManager = new FrameManager();
  return runWithFrameManager(frameManager, () => render.component(componentFn, props));
}, "renderAppVNode");
var mountReactiveApp = /* @__PURE__ */ __name((renderer, container, componentFn, props) => render.mount_reactive(renderer, container, () => render.component(componentFn, props)), "mountReactiveApp");
var hydrateReactiveApp = /* @__PURE__ */ __name((renderer, container, componentFn, props) => render.hydrate_reactive(renderer, container, () => render.component(componentFn, props)), "hydrateReactiveApp");
var mountTestingApp = /* @__PURE__ */ __name((harness, componentFn, props, hydrate = false) => {
  const renderer = harness.renderer ?? createDomRenderer({
...
var mountCustomElementHost = /* @__PURE__ */ __name((host, componentFn, options) => {
  ...
  const props = new Signal(buildCustomElementProps(host, options));
...
var defineCustomElementClass = /* @__PURE__ */ __name((tagName, componentFn, options) => {
  var _a4;
  const BaseCtor = options?.baseClass ?? globalThis.HTMLElement ?? class {
  };
  const registry = options?.registry ?? globalThis.customElements;
  const CustomElement = (_a4 = class extends BaseCtor {
    constructor() {
      super(...arguments);
  ...
  }, __name(_a4, "LuminaCustomElement"), _a4);
  if (registry?.define) {
...
var drawCanvasVNode = /* @__PURE__ */ __name((ctx, node, state2) => {
  ...
  if (node.kind === "live_text") {
    if (ctx.fillText) ctx.fillText(node.signal ? String(node.signal.get()) : "", state2.x, state2.y);
    return state2.y + state2.lineHeight;
  }
  if (node.kind === "index_list") {
    let y2 = state2.y;
...
var vnodeToTerminal = /* @__PURE__ */ __name((node, depth = 0) => {
  ...
  if (node.kind === "live_text") {
    return [
      `${indent}${node.signal ? String(node.signal.get()) : ""}`
    ];
  }
  if (node.kind === "index_list") {
...
var devtoolsRootIds = /* @__PURE__ */ new WeakMap();
...
var reorderChildren = /* @__PURE__ */ __name((container, children2, eventStore, portalStore, liveTextStore, options) => {
  ...
  const sequence = children2.map((child) => currentOrder.get(child) ?? -1);
  const keepIndices = longestIncreasingSubsequenceIndices(sequence);
  const keepIndexSet = new Set(keepIndices);
  let anchor = null;
  for (let index = children2.length - 1; index >= 0; index -= 1) {
    const nextChild = children2[index];
...
var snapshotFrame = /* @__PURE__ */ __name((frame) => ({
  id: frame.id,
...
__name(snapshotDevtools, "snapshotDevtools");
var subscribeDevtools = /* @__PURE__ */ __name((listener) => {
...
var installLuminaDevtools = /* @__PURE__ */ __name((key = "__LUMINA_DEVTOOLS__") => {
  const globalRecord = globalThis;
  const handle = {
    version: "beta",
    snapshot: /* @__PURE__ */ __name(() => snapshotDevtools(), "snapshot"),
    subscribe: /* @__PURE__ */ __name((listener) => subscribeDevtools(listener), "subscribe")
  };
...
var renderTransitionPresence = /* @__PURE__ */ __name((open, props, durationMs, children2) => {
  ...
  return vnodeElement("div", currentProps, resolveChildrenInput(children2));
}, "renderTransitionPresence");
...
var renderSsgPage = /* @__PURE__ */ __name((body, options) => {
  const normalized = coerceSsgPageOptions(options);
  const bodyContent = isVNode(body) ? renderToString(body) : Array.isArray(body) || body && typeof body === "object" ? renderToString(coerceRenderableToVNode(body)) : String(body ?? "");
  const head = [
    '<meta charset="utf-8">',
  ...
  const hydrateScript = normalized.hydrateModule ? `<script type="module" src="${escapeHtml(normalized.hydrateModule)}"></script>` : "";
...
var writeSsgPage = /* @__PURE__ */ __name((filePath, body, options) => {
  ...
  fsModule.mkdirSync(dirnamePathBasic(resolvedPath), {
    recursive: true
  });
...
var checkboxContext = createContextToken();
var checkboxRootIds = /* @__PURE__ */ new WeakMap();
var nextCheckboxRootId = 1;
var radioGroupContext = createContextToken();
var radioItemContext = createContextToken();
var radioRootIds = /* @__PURE__ */ new WeakMap();
var nextRadioRootId = 1;
var selectContext = createContextToken();
var selectItemContext = createContextToken();
var selectRootIds = /* @__PURE__ */ new WeakMap();
var selectAnchorTargets = /* @__PURE__ */ new WeakMap();
var selectRestoreTargets = /* @__PURE__ */ new WeakMap();
var nextSelectRootId = 1;
var comboboxContext = createContextToken();
var comboboxItemContext = createContextToken();
var comboboxRootIds = /* @__PURE__ */ new WeakMap();
var comboboxAnchorTargets = /* @__PURE__ */ new WeakMap();
var comboboxRestoreTargets = /* @__PURE__ */ new WeakMap();
var nextComboboxRootId = 1;
var multiselectContext = createContextToken();
var multiselectItemContext = createContextToken();
var multiselectRootIds = /* @__PURE__ */ new WeakMap();
var multiselectAnchorTargets = /* @__PURE__ */ new WeakMap();
var multiselectRestoreTargets = /* @__PURE__ */ new WeakMap();
var nextMultiselectRootId = 1;
...
var getCheckboxBaseId = /* @__PURE__ */ __name((signal) => {
  const existing = checkboxRootIds.get(signal);
  if (existing) return existing;
  const next = `lumina-checkbox-${nextCheckboxRootId++}`;
  checkboxRootIds.set(signal, next);
  return next;
}, "getCheckboxBaseId");
var getRadioBaseId = /* @__PURE__ */ __name((signal) => {
  const existing = radioRootIds.get(signal);
  if (existing) return existing;
  const next = `lumina-radio-${nextRadioRootId++}`;
  radioRootIds.set(signal, next);
  return next;
}, "getRadioBaseId");
...
var tooltipContext = createContextToken();
var tooltipRootIds = /* @__PURE__ */ new WeakMap();
var tooltipAnchorTargets = /* @__PURE__ */ new WeakMap();
var nextTooltipRootId = 1;
var toastContext = createContextToken();
var toastRootIds = /* @__PURE__ */ new WeakMap();
var toastTimers = /* @__PURE__ */ new WeakMap();
var nextToastRootId = 1;
...
var getTooltipBaseId = /* @__PURE__ */ __name((signal) => {
  const existing = tooltipRootIds.get(signal);
  if (existing) return existing;
  const next = `lumina-tooltip-${nextTooltipRootId++}`;
  tooltipRootIds.set(signal, next);
  return next;
}, "getTooltipBaseId");
var getToastBaseId = /* @__PURE__ */ __name((signal) => {
  const existing = toastRootIds.get(signal);
  if (existing) return existing;
  const next = `lumina-toast-${nextToastRootId++}`;
  toastRootIds.set(signal, next);
  return next;
}, "getToastBaseId");
...
var getSelectBaseId = /* @__PURE__ */ __name((signal) => {
  const existing = selectRootIds.get(signal);
  if (existing) return existing;
  const next = `lumina-select-${nextSelectRootId++}`;
  selectRootIds.set(signal, next);
  return next;
}, "getSelectBaseId");
var getComboboxBaseId = /* @__PURE__ */ __name((signal) => {
  const existing = comboboxRootIds.get(signal);
  if (existing) return existing;
  const next = `lumina-combobox-${nextComboboxRootId++}`;
  comboboxRootIds.set(signal, next);
  return next;
}, "getComboboxBaseId");
var getMultiselectBaseId = /* @__PURE__ */ __name((signal) => {
  const existing = multiselectRootIds.get(signal);
  if (existing) return existing;
  const next = `lumina-multiselect-${nextMultiselectRootId++}`;
  multiselectRootIds.set(signal, next);
  return next;
...
var getTooltipIds = /* @__PURE__ */ __name((ctx) => ({
  triggerId: `${ctx.baseId}-trigger`,
  contentId: `${ctx.baseId}-content`
}), "getTooltipIds");
var getToastIds = /* @__PURE__ */ __name((ctx) => ({
  contentId: `${ctx.baseId}-content`,
  titleId: `${ctx.baseId}-title`,
  descriptionId: `${ctx.baseId}-description`
}), "getToastIds");
...
var getSelectIds = /* @__PURE__ */ __name((ctx) => ({
  triggerId: `${ctx.baseId}-trigger`,
  contentId: `${ctx.baseId}-content`
}), "getSelectIds");
var getComboboxIds = /* @__PURE__ */ __name((ctx) => ({
  inputId: `${ctx.baseId}-input`,
  contentId: `${ctx.baseId}-content`
}), "getComboboxIds");
var getMultiselectIds = /* @__PURE__ */ __name((ctx) => ({
  triggerId: `${ctx.baseId}-trigger`,
  contentId: `${ctx.baseId}-content`
}), "getMultiselectIds");
var getCheckboxIds = /* @__PURE__ */ __name((ctx) => ({
...
var getRadioItemId = /* @__PURE__ */ __name((ctx, value) => `${ctx.baseId}-item-${normalizeTabsPart(value)}`, "getRadioItemId");
var getSelectItemId = /* @__PURE__ */ __name((ctx, value) => `${ctx.baseId}-item-${normalizeTabsPart(value)}`, "getSelectItemId");
var getComboboxItemId = /* @__PURE__ */ __name((ctx, value) => `${ctx.baseId}-item-${normalizeTabsPart(value)}`, "getComboboxItemId");
var getMultiselectItemId = /* @__PURE__ */ __name((ctx, value) => `${ctx.baseId}-item-${normalizeTabsPart(value)}`, "getMultiselectItemId");
var getRadioIndicatorId = /* @__PURE__ */ __name((itemId) => `${itemId}-indicator`, "getRadioIndicatorId");
var getSelectIndicatorId = /* @__PURE__ */ __name((itemId) => `${itemId}-indicator`, "getSelectIndicatorId");
var getComboboxIndicatorId = /* @__PURE__ */ __name((itemId) => `${itemId}-indicator`, "getComboboxIndicatorId");
var getMultiselectIndicatorId = /* @__PURE__ */ __name((itemId) => `${itemId}-indicator`, "getMultiselectIndicatorId");
...
var clearToastTimer = /* @__PURE__ */ __name((signal) => {
  const key = signal;
...
var scheduleToastTimer = /* @__PURE__ */ __name((ctx, duration) => {
...
var restoreSelectFocus = /* @__PURE__ */ __name((ctx) => {
  const key = ctx.open;
...
var restoreComboboxFocus = /* @__PURE__ */ __name((ctx) => {
  const key = ctx.open;
...
var restoreMultiselectFocus = /* @__PURE__ */ __name((ctx) => {
  const key = ctx.open;
...
var registerRadioValue = /* @__PURE__ */ __name((ctx, value) => {
  if (!ctx.order.includes(value)) {
    ctx.order.push(value);
  }
}, "registerRadioValue");
var registerSelectValue = /* @__PURE__ */ __name((ctx, value) => {
  if (!ctx.order.includes(value)) {
    ctx.order.push(value);
  }
}, "registerSelectValue");
var registerComboboxValue = /* @__PURE__ */ __name((ctx, value) => {
  if (!ctx.order.includes(value)) {
    ctx.order.push(value);
  }
}, "registerComboboxValue");
var registerMultiselectValue = /* @__PURE__ */ __name((ctx, value) => {
  if (!ctx.order.includes(value)) {
    ctx.order.push(value);
  }
...
var getRadioNavigationTarget = /* @__PURE__ */ __name((ctx, current, key) => {
  if (ctx.order.length === 0) return null;
  const currentIndex = Math.max(0, ctx.order.indexOf(current));
  if (key === "Home") {
    return ctx.order[0] ?? null;
...
var getSelectNavigationTarget = /* @__PURE__ */ __name((ctx, current, key) => {
  if (ctx.order.length === 0) return null;
  const currentIndex = Math.max(0, ctx.order.indexOf(current));
  if (key === "Home") {
    return ctx.order[0] ?? null;
...
var getComboboxNavigationTarget = /* @__PURE__ */ __name((ctx, current, key) => {
  if (ctx.order.length === 0) return null;
  const currentIndex = Math.max(0, ctx.order.indexOf(current));
  if (key === "Home") {
    return ctx.order[0] ?? null;
...
var getMultiselectNavigationTarget = /* @__PURE__ */ __name((ctx, current, key) => {
  if (ctx.order.length === 0) return null;
  const currentIndex = Math.max(0, ctx.order.indexOf(current));
  if (key === "Home") {
    return ctx.order[0] ?? null;
...
var focusRadioItem = /* @__PURE__ */ __name((documentLike, ctx, value, fallbackRoot) => {
...
var focusSelectItem = /* @__PURE__ */ __name((documentLike, ctx, value, fallbackRoot) => {
...
var focusComboboxItem = /* @__PURE__ */ __name((documentLike, ctx, value, fallbackRoot) => {
...
var focusMultiselectItem = /* @__PURE__ */ __name((documentLike, ctx, value, fallbackRoot) => {
...
var closeSelect = /* @__PURE__ */ __name((ctx) => {
  ctx.open.set(false);
  restoreSelectFocus(ctx);
}, "closeSelect");
var closeCombobox = /* @__PURE__ */ __name((ctx) => {
  ctx.open.set(false);
  restoreComboboxFocus(ctx);
}, "closeCombobox");
var closeMultiselect = /* @__PURE__ */ __name((ctx) => {
  ctx.open.set(false);
  restoreMultiselectFocus(ctx);
}, "closeMultiselect");
var readStringSelection = /* @__PURE__ */ __name((value) => Array.isArray(value) ? value.filter((entry) => typeof entry === "string") : [], "readStringSelection");
var toggleMultiselectValue = /* @__PURE__ */ __name((ctx, value) => {
...
var getTooltipAnchorRect = /* @__PURE__ */ __name((ctx) => {
  const anchor = tooltipAnchorTargets.get(ctx.open);
  if (!anchor || typeof anchor.getBoundingClientRect !== "function") return null;
  const raw = anchor.getBoundingClientRect();
  const left = readNumericRectValue(raw?.left) ?? 0;
  const top = readNumericRectValue(raw?.top) ?? 0;
...
var getSelectAnchorRect = /* @__PURE__ */ __name((ctx) => {
  const anchor = selectAnchorTargets.get(ctx.open);
  if (!anchor || typeof anchor.getBoundingClientRect !== "function") return null;
  const raw = anchor.getBoundingClientRect();
  const left = readNumericRectValue(raw?.left) ?? 0;
  const top = readNumericRectValue(raw?.top) ?? 0;
...
var getComboboxAnchorRect = /* @__PURE__ */ __name((ctx) => {
  const anchor = comboboxAnchorTargets.get(ctx.open);
  if (!anchor || typeof anchor.getBoundingClientRect !== "function") return null;
  const raw = anchor.getBoundingClientRect();
  const left = readNumericRectValue(raw?.left) ?? 0;
  const top = readNumericRectValue(raw?.top) ?? 0;
...
var getMultiselectAnchorRect = /* @__PURE__ */ __name((ctx) => {
  const anchor = multiselectAnchorTargets.get(ctx.open);
  if (!anchor || typeof anchor.getBoundingClientRect !== "function") return null;
  const raw = anchor.getBoundingClientRect();
  const left = readNumericRectValue(raw?.left) ?? 0;
  const top = readNumericRectValue(raw?.top) ?? 0;
...
var pickToastDuration = /* @__PURE__ */ __name((props) => {
  const value = props?.duration;
  return typeof value === "number" && Number.isFinite(value) ? value : 0;
}, "pickToastDuration");
var omitToastControlProps = /* @__PURE__ */ __name((props) => {
  if (!props) return void 0;
  const next = {
    ...props
  };
...
var render = {
  ...
  render_app: /* @__PURE__ */ __name((componentFn, props) => renderAppVNode(componentFn, props), "render_app"),
  render_to_string_app: /* @__PURE__ */ __name((componentFn, props) => renderToString(renderAppVNode(componentFn, props)), "render_to_string_app"),
  ...
  transition_presence: /* @__PURE__ */ __name((open, props, durationMs, renderChildren) => renderTransitionPresence(open, props, durationMs, renderChildren), "transition_presence"),
  ...
  createResource: /* @__PURE__ */ __name((key, loader, options) => render.resource_create(key, loader, options), "createResource"),
  renderApp: /* @__PURE__ */ __name((componentFn, props) => render.render_app(componentFn, props), "renderApp"),
  renderToStringApp: /* @__PURE__ */ __name((componentFn, props) => render.render_to_string_app(componentFn, props), "renderToStringApp"),
  transitionPresence: /* @__PURE__ */ __name((open, props, durationMs, renderChildren) => render.transition_presence(open, props, durationMs, renderChildren), "transitionPresence"),
  resourceStatus: /* @__PURE__ */ __name((resource) => render.resource_status(resource), "resourceStatus"),
  resourceData: /* @__PURE__ */ __name((resource) => render.resource_data(resource), "resourceData"),
  resourceError: /* @__PURE__ */ __name((resource) => render.resource_error(resource), "resourceError"),
  resourceRead: /* @__PURE__ */ __name((resource) => render.resource_read(resource), "resourceRead"),
  resourceRefresh: /* @__PURE__ */ __name((resource) => render.resource_refresh(resource), "resourceRefresh"),
  resourceInvalidate: /* @__PURE__ */ __name((resource) => render.resource_invalidate(resource), "resourceInvalidate"),
  resourceMutate: /* @__PURE__ */ __name((resource, value) => render.resource_mutate(resource, value), "resourceMutate"),
  errorBoundary: /* @__PURE__ */ __name((fallback, renderChildren) => render.error_boundary(fallback, renderChildren), "errorBoundary"),
  mountApp: /* @__PURE__ */ __name((renderer, container, componentFn, props) => render.mount_app(renderer, container, componentFn, props), "mountApp"),
  hydrateApp: /* @__PURE__ */ __name((renderer, container, componentFn, props) => render.hydrate_app(renderer, container, componentFn, props), "hydrateApp"),
  testingCreateDomHarness: /* @__PURE__ */ __name(() => render.testing_create_dom_harness(), "testingCreateDomHarness"),
  testingMountApp: /* @__PURE__ */ __name((harness, componentFn, props) => render.testing_mount_app(harness, componentFn, props), "testingMountApp"),
  testingHydrateApp: /* @__PURE__ */ __name((harness, componentFn, props) => render.testing_hydrate_app(harness, componentFn, props), "testingHydrateApp"),
  testingContainer: /* @__PURE__ */ __name((harness) => render.testing_container(harness), "testingContainer"),
  testingBody: /* @__PURE__ */ __name((harness) => render.testing_body(harness), "testingBody"),
  testingGetById: /* @__PURE__ */ __name((harness, id) => render.testing_get_by_id(harness, id), "testingGetById"),
  testingGetByText: /* @__PURE__ */ __name((scope, value) => render.testing_get_by_text(scope, value), "testingGetByText"),
  testingGetByRole: /* @__PURE__ */ __name((scope, role) => {
    const matches = render.testing_query_all_by_role(scope, role);
    return matches[0] ?? null;
  }, "testingGetByRole"),
  testingQueryAllByRole: /* @__PURE__ */ __name((scope, role) => render.testing_query_all_by_role(scope, role), "testingQueryAllByRole"),
  testingTextContent: /* @__PURE__ */ __name((node) => render.testing_text_content(node), "testingTextContent"),
  testingClick: /* @__PURE__ */ __name((node) => render.testing_click(node), "testingClick"),
  testingInput: /* @__PURE__ */ __name((node, value) => render.testing_input(node, value), "testingInput"),
  testingChangeChecked: /* @__PURE__ */ __name((node, checked) => render.testing_change_checked(node, checked), "testingChangeChecked"),
  testingKeydown: /* @__PURE__ */ __name((node, key, shiftKey) => render.testing_keydown(node, key, shiftKey), "testingKeydown"),
  testingSubmit: /* @__PURE__ */ __name((node) => render.testing_submit(node), "testingSubmit"),
  devtoolsSnapshot: /* @__PURE__ */ __name(() => render.devtools_snapshot(), "devtoolsSnapshot"),
  installDevtools: /* @__PURE__ */ __name((key) => render.install_devtools(key), "installDevtools"),
  ssgPage: /* @__PURE__ */ __name((body, options) => render.ssg_page(body, options), "ssgPage"),
  ssgRenderApp: /* @__PURE__ */ __name((componentFn, props, options) => render.ssg_render_app(componentFn, props, options), "ssgRenderApp"),
  ssgWritePage: /* @__PURE__ */ __name((filePath, body, options) => render.ssg_write_page(filePath, body, options), "ssgWritePage"),
  ssgWriteApp: /* @__PURE__ */ __name((filePath, componentFn, props, options) => render.ssg_write_app(filePath, componentFn, props, options), "ssgWriteApp"),
  devtools_snapshot: /* @__PURE__ */ __name(() => snapshotDevtools(), "devtools_snapshot"),
  install_devtools: /* @__PURE__ */ __name((key) => installLuminaDevtools(key), "install_devtools"),
  ssg_page: /* @__PURE__ */ __name((body, options) => renderSsgPage(body, options), "ssg_page"),
  ssg_render_app: /* @__PURE__ */ __name((componentFn, props, options) => renderSsgPage(render.render_app(componentFn, props), options), "ssg_render_app"),
  ssg_write_page: /* @__PURE__ */ __name((filePath, body, options) => writeSsgPage(filePath, body, options), "ssg_write_page"),
  ssg_write_app: /* @__PURE__ */ __name((filePath, componentFn, props, options) => writeSsgPage(filePath, render.render_app(componentFn, props), options), "ssg_write_app"),
  mountCustomElement: /* @__PURE__ */ __name((host, componentFn, options) => render.mount_custom_element(host, componentFn, options), "mountCustomElement"),
  defineCustomElement: /* @__PURE__ */ __name((tagName, componentFn, options) => render.define_custom_element(tagName, componentFn, options), "defineCustomElement"),
  ...
  tooltip_trigger: /* @__PURE__ */ __name((props, children2 = []) => {
    ...
    return vnodeElement("button", mergeProps({
      ...
      onMouseEnter: /* @__PURE__ */ __name((event) => {
        const target = getFocusTargetFromEvent(event);
        if (target) {
          tooltipAnchorTargets.set(ctx.open, target);
        }
        ctx.open.set(true);
      }, "onMouseEnter"),
  ...
  toastRoot: /* @__PURE__ */ __name((open, renderChildren) => render.toast_root(open, renderChildren), "toastRoot"),
  toastPortal: /* @__PURE__ */ __name((children2 = []) => render.toast_portal(children2), "toastPortal"),
  toastContent: /* @__PURE__ */ __name((props, children2 = []) => render.toast_content(props, children2), "toastContent"),
  toastTitle: /* @__PURE__ */ __name((props, children2 = []) => render.toast_title(props, children2), "toastTitle"),
  toastDescription: /* @__PURE__ */ __name((props, children2 = []) => render.toast_description(props, children2), "toastDescription"),
  toastClose: /* @__PURE__ */ __name((props, children2 = []) => render.toast_close(props, children2), "toastClose"),
  ...
  select_trigger: /* @__PURE__ */ __name((props, children2 = []) => {
    ...
    return vnodeElement("button", mergeProps({
      ...
      onClick: /* @__PURE__ */ __name((event) => {
        const target = getFocusTargetFromEvent(event);
        if (target) {
          selectRestoreTargets.set(ctx.open, target);
          selectAnchorTargets.set(ctx.open, target);
        }
        const nextOpen = !ctx.open.get();
        ctx.open.set(nextOpen);
        if (!nextOpen) {
  ...
  selectRoot: /* @__PURE__ */ __name((open, value, renderChildren) => render.select_root(open, value, renderChildren), "selectRoot"),
  selectPortal: /* @__PURE__ */ __name((children2 = []) => render.select_portal(children2), "selectPortal"),
  selectTrigger: /* @__PURE__ */ __name((props, children2 = []) => render.select_trigger(props, children2), "selectTrigger"),
  selectContent: /* @__PURE__ */ __name((props, children2 = []) => render.select_content(props, children2), "selectContent"),
  selectItem: /* @__PURE__ */ __name((value, props, renderChildren) => render.select_item(value, props, renderChildren), "selectItem"),
  selectIndicator: /* @__PURE__ */ __name((props, children2 = []) => render.select_indicator(props, children2), "selectIndicator"),
  ...
  combobox_input: /* @__PURE__ */ __name((props, children2 = []) => {
    ...
    return vnodeElement("input", mergeProps({
      ...
      onInput: /* @__PURE__ */ __name((event) => {
        const target = getFocusTargetFromEvent(event);
        if (target) {
          comboboxRestoreTargets.set(ctx.open, target);
          comboboxAnchorTargets.set(ctx.open, target);
        }
  ...
  comboboxRoot: /* @__PURE__ */ __name((open, value, query2, renderChildren) => render.combobox_root(open, value, query2, renderChildren), "comboboxRoot"),
  comboboxPortal: /* @__PURE__ */ __name((children2 = []) => render.combobox_portal(children2), "comboboxPortal"),
  comboboxInput: /* @__PURE__ */ __name((props, children2 = []) => render.combobox_input(props, children2), "comboboxInput"),
  comboboxContent: /* @__PURE__ */ __name((props, children2 = []) => render.combobox_content(props, children2), "comboboxContent"),
  comboboxItem: /* @__PURE__ */ __name((value, props, renderChildren) => render.combobox_item(value, props, renderChildren), "comboboxItem"),
  comboboxIndicator: /* @__PURE__ */ __name((props, children2 = []) => render.combobox_indicator(props, children2), "comboboxIndicator"),
  ...
  multiselect_trigger: /* @__PURE__ */ __name((props, children2 = []) => {
    ...
    return vnodeElement("button", mergeProps({
      ...
      onClick: /* @__PURE__ */ __name((event) => {
        const target = getFocusTargetFromEvent(event);
        if (target) {
          multiselectRestoreTargets.set(ctx.open, target);
          multiselectAnchorTargets.set(ctx.open, target);
        }
        const nextOpen = !ctx.open.get();
        ctx.open.set(nextOpen);
        if (!nextOpen) {
  ...
  multiselectRoot: /* @__PURE__ */ __name((open, values, renderChildren) => render.multiselect_root(open, values, renderChildren), "multiselectRoot"),
  multiselectPortal: /* @__PURE__ */ __name((children2 = []) => render.multiselect_portal(children2), "multiselectPortal"),
  multiselectTrigger: /* @__PURE__ */ __name((props, children2 = []) => render.multiselect_trigger(props, children2), "multiselectTrigger"),
  multiselectContent: /* @__PURE__ */ __name((props, children2 = []) => render.multiselect_content(props, children2), "multiselectContent"),
  multiselectItem: /* @__PURE__ */ __name((value, props, renderChildren) => render.multiselect_item(value, props, renderChildren), "multiselectItem"),
  multiselectIndicator: /* @__PURE__ */ __name((props, children2 = []) => render.multiselect_indicator(props, children2), "multiselectIndicator"),
  ...
  checkboxRoot: /* @__PURE__ */ __name((checked, props, renderChildren) => render.checkbox_root(checked, props, renderChildren), "checkboxRoot"),
  checkboxIndicator: /* @__PURE__ */ __name((props, children2 = []) => render.checkbox_indicator(props, children2), "checkboxIndicator"),
  ...
  radioGroup: /* @__PURE__ */ __name((value, props, renderChildren) => render.radio_group(value, props, renderChildren), "radioGroup"),
  radioItem: /* @__PURE__ */ __name((value, props, renderChildren) => render.radio_item(value, props, renderChildren), "radioItem"),
  radioIndicator: /* @__PURE__ */ __name((props, children2 = []) => render.radio_indicator(props, children2), "radioIndicator"),
  ...
  tooltipRoot: /* @__PURE__ */ __name((open, renderChildren) => render.tooltip_root(open, renderChildren), "tooltipRoot"),
  tooltipPortal: /* @__PURE__ */ __name((children2 = []) => render.tooltip_portal(children2), "tooltipPortal"),
  tooltipTrigger: /* @__PURE__ */ __name((props, children2 = []) => render.tooltip_trigger(props, children2), "tooltipTrigger"),
  tooltipContent: /* @__PURE__ */ __name((props, children2 = []) => render.tooltip_content(props, children2), "tooltipContent"),
  ...
  props_checked: /* @__PURE__ */ __name((checked) => ({
    checked
  }), "props_checked"),
  props_type: /* @__PURE__ */ __name((type) => ({
    type
  }), "props_type"),
  props_name: /* @__PURE__ */ __name((name) => ({
    name
  }), "props_name"),
  ...
  props_on_checked_change: /* @__PURE__ */ __name((handler) => ({
    onChange: /* @__PURE__ */ __name((e) => handler(!!e.target?.checked), "onChange")
  }), "props_on_checked_change"),
  props_on_submit: /* @__PURE__ */ __name((handler) => ({
  ...
  mount_app: /* @__PURE__ */ __name((renderer, container, componentFn, props) => mountReactiveApp(renderer, container, componentFn, props), "mount_app"),
  hydrate_app: /* @__PURE__ */ __name((renderer, container, componentFn, props) => hydrateReactiveApp(renderer, container, componentFn, props), "hydrate_app"),
  testing_create_dom_harness: /* @__PURE__ */ __name(() => {
    const harness = createTestingDomHarness();
    harness.renderer = createDomRenderer({
      document: harness.document
    });
    return harness;
  }, "testing_create_dom_harness"),
  testing_mount_app: /* @__PURE__ */ __name((harness, componentFn, props) => mountTestingApp(harness, componentFn, props, false), "testing_mount_app"),
  testing_hydrate_app: /* @__PURE__ */ __name((harness, componentFn, props) => mountTestingApp(harness, componentFn, props, true), "testing_hydrate_app"),
  ...
  mount_custom_element: /* @__PURE__ */ __name((host, componentFn, options) => mountCustomElementHost(host, componentFn, options), "mount_custom_element"),
  define_custom_element: /* @__PURE__ */ __name((tagName, componentFn, options) => defineCustomElementClass(tagName, componentFn, options), "define_custom_element"),
...
var renderApp = /* @__PURE__ */ __name((componentFn, props) => render.render_app(componentFn, props), "renderApp");
var renderToStringApp = /* @__PURE__ */ __name((componentFn, props) => render.render_to_string_app(componentFn, props), "renderToStringApp");
...
var createResource = /* @__PURE__ */ __name((key, loader, options) => render.resource_create(key, loader, options), "createResource");
var resourceStatus = /* @__PURE__ */ __name((resource) => render.resource_status(resource), "resourceStatus");
var resourceData = /* @__PURE__ */ __name((resource) => render.resource_data(resource), "resourceData");
var resourceError = /* @__PURE__ */ __name((resource) => render.resource_error(resource), "resourceError");
var resourceRead = /* @__PURE__ */ __name((resource) => render.resource_read(resource), "resourceRead");
var resourceRefresh = /* @__PURE__ */ __name((resource) => render.resource_refresh(resource), "resourceRefresh");
var resourceInvalidate = /* @__PURE__ */ __name((resource) => render.resource_invalidate(resource), "resourceInvalidate");
var resourceMutate = /* @__PURE__ */ __name((resource, value) => render.resource_mutate(resource, value), "resourceMutate");
var suspense = /* @__PURE__ */ __name((fallback, renderChildren) => render.suspense(fallback, renderChildren), "suspense");
var errorBoundary = /* @__PURE__ */ __name((fallback, renderChildren) => render.error_boundary(fallback, renderChildren), "errorBoundary");
var mountApp = /* @__PURE__ */ __name((renderer, container, componentFn, props) => render.mount_app(renderer, container, componentFn, props), "mountApp");
var hydrateApp = /* @__PURE__ */ __name((renderer, container, componentFn, props) => render.hydrate_app(renderer, container, componentFn, props), "hydrateApp");
var testingCreateDomHarness = /* @__PURE__ */ __name(() => render.testing_create_dom_harness(), "testingCreateDomHarness");
var testingMountApp = /* @__PURE__ */ __name((harness, componentFn, props) => render.testing_mount_app(harness, componentFn, props), "testingMountApp");
var testingHydrateApp = /* @__PURE__ */ __name((harness, componentFn, props) => render.testing_hydrate_app(harness, componentFn, props), "testingHydrateApp");
var testingContainer = /* @__PURE__ */ __name((harness) => render.testing_container(harness), "testingContainer");
var testingBody = /* @__PURE__ */ __name((harness) => render.testing_body(harness), "testingBody");
var testingGetById = /* @__PURE__ */ __name((harness, id) => render.testing_get_by_id(harness, id), "testingGetById");
var testingTextContent = /* @__PURE__ */ __name((node) => render.testing_text_content(node), "testingTextContent");
var testingClick = /* @__PURE__ */ __name((node) => render.testing_click(node), "testingClick");
var testingInput = /* @__PURE__ */ __name((node, value) => render.testing_input(node, value), "testingInput");
var testingChangeChecked = /* @__PURE__ */ __name((node, checked) => render.testing_change_checked(node, checked), "testingChangeChecked");
var testingKeydown = /* @__PURE__ */ __name((node, key, shiftKey) => render.testing_keydown(node, key, shiftKey), "testingKeydown");
var testingSubmit = /* @__PURE__ */ __name((node) => render.testing_submit(node), "testingSubmit");
var mountCustomElement = /* @__PURE__ */ __name((host, componentFn, options) => render.mount_custom_element(host, componentFn, options), "mountCustomElement");
var defineCustomElement = /* @__PURE__ */ __name((tagName, componentFn, options) => render.define_custom_element(tagName, componentFn, options), "defineCustomElement");
...
var tooltipRoot = /* @__PURE__ */ __name((open, renderChildren) => render.tooltip_root(open, renderChildren), "tooltipRoot");
var tooltipPortal = /* @__PURE__ */ __name((children2 = []) => render.tooltip_portal(children2), "tooltipPortal");
var tooltipTrigger = /* @__PURE__ */ __name((props, children2 = []) => render.tooltip_trigger(props, children2), "tooltipTrigger");
var tooltipContent = /* @__PURE__ */ __name((props, children2 = []) => render.tooltip_content(props, children2), "tooltipContent");
var toastRoot = /* @__PURE__ */ __name((open, renderChildren) => render.toast_root(open, renderChildren), "toastRoot");
var toastPortal = /* @__PURE__ */ __name((children2 = []) => render.toast_portal(children2), "toastPortal");
var toastContent = /* @__PURE__ */ __name((props, children2 = []) => render.toast_content(props, children2), "toastContent");
var toastTitle = /* @__PURE__ */ __name((props, children2 = []) => render.toast_title(props, children2), "toastTitle");
var toastDescription = /* @__PURE__ */ __name((props, children2 = []) => render.toast_description(props, children2), "toastDescription");
var toastClose = /* @__PURE__ */ __name((props, children2 = []) => render.toast_close(props, children2), "toastClose");
...
var selectRoot = /* @__PURE__ */ __name((open, value, renderChildren) => render.select_root(open, value, renderChildren), "selectRoot");
var selectPortal = /* @__PURE__ */ __name((children2 = []) => render.select_portal(children2), "selectPortal");
var selectTrigger = /* @__PURE__ */ __name((props, children2 = []) => render.select_trigger(props, children2), "selectTrigger");
var selectContent = /* @__PURE__ */ __name((props, children2 = []) => render.select_content(props, children2), "selectContent");
var selectItem = /* @__PURE__ */ __name((value, props, renderChildren) => render.select_item(value, props, renderChildren), "selectItem");
var selectIndicator = /* @__PURE__ */ __name((props, children2 = []) => render.select_indicator(props, children2), "selectIndicator");
var comboboxRoot = /* @__PURE__ */ __name((open, value, query2, renderChildren) => render.combobox_root(open, value, query2, renderChildren), "comboboxRoot");
var comboboxPortal = /* @__PURE__ */ __name((children2 = []) => render.combobox_portal(children2), "comboboxPortal");
var comboboxInput = /* @__PURE__ */ __name((props, children2 = []) => render.combobox_input(props, children2), "comboboxInput");
var comboboxContent = /* @__PURE__ */ __name((props, children2 = []) => render.combobox_content(props, children2), "comboboxContent");
var comboboxItem = /* @__PURE__ */ __name((value, props, renderChildren) => render.combobox_item(value, props, renderChildren), "comboboxItem");
var comboboxIndicator = /* @__PURE__ */ __name((props, children2 = []) => render.combobox_indicator(props, children2), "comboboxIndicator");
var multiselectRoot = /* @__PURE__ */ __name((open, values, renderChildren) => render.multiselect_root(open, values, renderChildren), "multiselectRoot");
var multiselectPortal = /* @__PURE__ */ __name((children2 = []) => render.multiselect_portal(children2), "multiselectPortal");
var multiselectTrigger = /* @__PURE__ */ __name((props, children2 = []) => render.multiselect_trigger(props, children2), "multiselectTrigger");
var multiselectContent = /* @__PURE__ */ __name((props, children2 = []) => render.multiselect_content(props, children2), "multiselectContent");
var multiselectItem = /* @__PURE__ */ __name((value, props, renderChildren) => render.multiselect_item(value, props, renderChildren), "multiselectItem");
var multiselectIndicator = /* @__PURE__ */ __name((props, children2 = []) => render.multiselect_indicator(props, children2), "multiselectIndicator");
var checkboxRoot = /* @__PURE__ */ __name((checked, props, renderChildren) => render.checkbox_root(checked, props, renderChildren), "checkboxRoot");
var checkboxIndicator = /* @__PURE__ */ __name((props, children2 = []) => render.checkbox_indicator(props, children2), "checkboxIndicator");
var radioGroup = /* @__PURE__ */ __name((value, props, renderChildren) => render.radio_group(value, props, renderChildren), "radioGroup");
var radioItem = /* @__PURE__ */ __name((value, props, renderChildren) => render.radio_item(value, props, renderChildren), "radioItem");
var radioIndicator = /* @__PURE__ */ __name((props, children2 = []) => render.radio_indicator(props, children2), "radioIndicator");
...
var liveText = /* @__PURE__ */ __name((signal) => render.liveText(signal), "liveText");
var indexList = /* @__PURE__ */ __name((itemsSignal, renderItem) => render.indexList(itemsSignal, renderItem), "indexList");
var forList = /* @__PURE__ */ __name((itemsSignal, keyOf, renderItem) => render.forList(itemsSignal, keyOf, renderItem), "forList");
...
var props_checked = /* @__PURE__ */ __name((checked) => render.props_checked(checked), "props_checked");
var props_type = /* @__PURE__ */ __name((type) => render.props_type(type), "props_type");
var props_name = /* @__PURE__ */ __name((name) => render.props_name(name), "props_name");
...
var props_on_checked_change = /* @__PURE__ */ __name((handler) => render.props_on_checked_change(handler), "props_on_checked_change");
var props_on_submit = /* @__PURE__ */ __name((handler) => render.props_on_submit(handler), "props_on_submit");
...
var transitionPresence = /* @__PURE__ */ __name((open, props, durationMs, renderChildren) => render.transition_presence(open, props, durationMs, renderChildren), "transitionPresence");
var testingGetByText = /* @__PURE__ */ __name((scope, value) => render.testing_get_by_text(scope, value), "testingGetByText");
var testingGetByRole = /* @__PURE__ */ __name((scope, role) => render.testingGetByRole(scope, role), "testingGetByRole");
var testingQueryAllByRole = /* @__PURE__ */ __name((scope, role) => render.testing_query_all_by_role(scope, role), "testingQueryAllByRole");
var devtoolsSnapshot = /* @__PURE__ */ __name(() => render.devtools_snapshot(), "devtoolsSnapshot");
var installDevtools = /* @__PURE__ */ __name((key) => render.install_devtools(key), "installDevtools");
var ssgPage = /* @__PURE__ */ __name((body, options) => render.ssg_page(body, options), "ssgPage");
var ssgRenderApp = /* @__PURE__ */ __name((componentFn, props, options) => render.ssg_render_app(componentFn, props, options), "ssgRenderApp");
var ssgWritePage = /* @__PURE__ */ __name((filePath, body, options) => render.ssg_write_page(filePath, body, options), "ssgWritePage");
var ssgWriteApp = /* @__PURE__ */ __name((filePath, componentFn, props, options) => render.ssg_write_app(filePath, componentFn, props, options), "ssgWriteApp");
var reactive = {
  createSignal,
  get,
  set,
