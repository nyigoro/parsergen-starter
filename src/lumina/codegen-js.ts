...
class JSGenerator {
  ...
  emitProgram(node: LuminaProgram): void {
    ...
    if (this.includeRuntime) {
      ...
      this.builder.append('\n');
      this.builder.append('const webgpu = { is_available: () => false, request_adapter: async () => ({ $tag: "Err", $payload: "No webgpu runtime" }), request_device: async () => ({ $tag: "Err", $payload: "No webgpu runtime" }), compute_i32: async () => ({ $tag: "Err", $payload: "No webgpu runtime" }) };');
      this.builder.append('\n');
