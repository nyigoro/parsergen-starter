...
| LSP refactor code actions | Stable | Extract local/type alias/function, inline variable, promote-to-ref, split variable, trait stubs, change signature, move symbol, async/result rewrites, if/else↔match transforms, and collection call style rewrites; VS Code extension exposes move-symbol target picking and a webview-backed change-signature editor |
