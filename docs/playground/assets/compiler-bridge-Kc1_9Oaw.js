import{c as h,B as g}from"./compiler-analysis-C101JkUN.js";import{I as b,J as x}from"./compiler-core-Bu_R4ek3.js";import{g as N,a as v}from"./compiler-js-BJsNnWtF.js";import"./compiler-parser-D1zSBUbP.js";import"./compiler-stdlib-BPutdx6R.js";const R=`{\r
  let nodeId = 0;\r
...
`,T=`extern fn print(message: string) -> void from "console";\r
...
`,A=`import { router, reactive, render, str } from "@std"
...
`,I=""+new URL("lumina-runtime-0gbd7SGy.js",import.meta.url).href,m=/(^\s*import\b[\s\S]*?\bfrom\s+["']([^"']+)["'];?\s*)/gm,E=e=>{const n=[/^\s*pub\s+fn\s+([A-Za-z_][A-Za-z0-9_]*)/gm,/^\s*pub\s+let\s+([A-Za-z_][A-Za-z0-9_]*)/gm,/^\s*pub\s+struct\s+([A-Za-z_][A-Za-z0-9_]*)/gm,/^\s*pub\s+enum\s+([A-Za-z_][A-Za-z0-9_]*)/gm],t=new Set;for(const a of n)for(const r of e.matchAll(a))r[1]&&t.add(r[1]);return Array.from(t)},P=e=>{const n=new Set,t=/\bexport\s*\{([^}]*)\}/gm;for(const a of e.matchAll(t)){const r=a[1]?.split(",")??[];for(const o of r){const i=o.trim();if(!i)continue;const[,l=i]=i.split(/\s+as\s+/),s=l.trim();s&&n.add(s)}}return n},S=(e,n)=>{if(n.length===0)return e;const t=P(e),a=Array.from(new Set(n)).filter(r=>!t.has(r));return a.length===0?e:`${e.trimEnd()}
export { ${a.join(", ")} };
`},V=e=>{const n=new Set,t=/\bimport\s*\{([^}]*)\}\s*from\s+["'][^"']*lumina-runtime\.js["']/gm;for(const a of e.matchAll(t)){const r=a[1]?.split(",")??[];for(const o of r){const i=o.trim();if(!i)continue;const[,l=i]=i.split(/\s+as\s+/),s=l.trim();s&&n.add(s)}}return n},k=(e,n)=>{if(n.size===0)return e;const t=e.match(/^(\s*import\s*\{)([^}]*)(\}\s*from\s+["'][^"']+["'];?\s*)$/s);if(!t)return e;const a=t[2].split(",").map(r=>r.trim()).filter(r=>{if(!r)return!1;const[,o]=r.split(/\s+as\s+/),i=(o??r).trim();return!n.has(i)});return a.length===0?null:`${t[1]} ${a.join(", ")} ${t[3]}`.trim()},M=(e,n,t,a)=>{const r=[];let o;for(m.lastIndex=0;(o=m.exec(t))!==null;){const i=o[1],l=o[2],s=e.resolveImportUri(n,l);if(!e.getDocumentText(s))continue;const u=k(i.trim(),l.startsWith("@std/")?a:new Set);u&&r.push({resolvedUri:s,statement:u})}return r},w=e=>{const n=new Map,t=new Set,a=r=>{if(n.has(r))return;if(t.has(r))throw new Error(`Circular runnable module dependency detected for ${r}`);const o=e.project.getDocumentText(r),i=e.project.getDocumentAst(r);if(!o||!i)throw new Error(`Missing runnable module source for ${r}`);t.add(r);const l=N(i,{target:"esm",includeRuntime:!0,sourceMap:!1,sourceFile:r,sourceContent:o}).code.replace(/from\s+["']\.\/lumina-runtime\.js["']/g,`from ${JSON.stringify(e.runtimeUrl)}`),s=M(e.project,r,o,V(l));for(const u of s)a(u.resolvedUri);n.set(r,{uri:r,code:S(l,E(o)),sourceImports:s}),t.delete(r)};return a(e.entryUri),{entryUri:e.entryUri,modules:Array.from(n.values())}},C=1,L=new URL(I,import.meta.url).href,B=new Set([".lm",".lumina",".lum"]),U=e=>{const n=e.lastIndexOf("/"),t=n>=0?e.slice(n+1):e,a=t.lastIndexOf(".");return a>=0?t.slice(a):""},y=e=>B.has(U(e));let p=null;const z=()=>(p||(p={parser:h(R,{cache:!0})}),p),F=e=>{const t=e.replace(/^\uFEFF/,"").replace(/\r\n?/g,`
`).split(`
`),a=[];let r=0;for(const o of t){const i=o.replace(/[ \t]+$/g,"");if(i.length===0){r+=1,r<=C&&a.push("");continue}r=0,a.push(i)}for(;a.length>0&&a[a.length-1]==="";)a.pop();return`${a.join(`
`)}
`},D=(e,n,t=120)=>{const r=n.replace(/\r\n?/g,`
`).split(`
`),o=[];for(let i=0;i<r.length;i+=1){const l=r[i],s=i+1,u=l.match(/[ \t]+$/);u&&o.push({severity:"warning",message:"Trailing whitespace",line:s,column:u.index+1,code:"LINT-TRAILING-WS",fileUri:e});const d=l.indexOf("	");d>=0&&o.push({severity:"warning",message:"Tab indentation found; use spaces",line:s,column:d+1,code:"LINT-TAB-INDENT",fileUri:e}),l.length>t&&o.push({severity:"warning",message:`Line exceeds ${t} characters`,line:s,column:t+1,code:"LINT-LINE-LENGTH",fileUri:e})}return o},q=(e,n,t)=>{const a=t.map(o=>({severity:o.severity,message:o.message,line:o.location?.start?.line,column:o.location?.start?.column,code:o.code,fileUri:e})),r=D(e,n);return[...a,...r]},$=e=>!!(e&&typeof e=="object"&&Array.isArray(e.body)&&e.body.some(n=>n.type==="FnDecl"&&n.name==="main")),K=(e,n)=>{const t=new Map([["@std/router",A]]);for(const r of n)t.set(r.uri,r.text);const a=new g(e.parser,{preludeText:T,virtualFiles:t});for(const r of n)y(r.uri)&&a.addOrUpdateDocument(r.uri,r.text,1);return a},W=e=>{try{const n=z(),t=K(n,e.files),a=e.files.filter(c=>y(c.uri)).flatMap(c=>q(c.uri,c.text,t.getDiagnostics(c.uri)));if(a.some(c=>c.severity==="error"))return{ok:!1,js:"",runnableJs:"",runnableEntryUri:null,runnableModules:[],hasMain:!1,diagnostics:a,entryUri:e.entryUri,graphEdges:0,graphNodes:0};const o=t.getDocumentAst(e.entryUri);if(!o)return{ok:!1,js:"",runnableJs:"",runnableEntryUri:null,runnableModules:[],hasMain:!1,diagnostics:[{severity:"error",message:`No AST produced for ${e.entryUri}`,fileUri:e.entryUri}],entryUri:e.entryUri,graphEdges:0,graphNodes:0};const i=b(o),l=x(i),s=l?v(l).code:"// No JavaScript output generated.",u=w({project:t,entryUri:e.entryUri,runtimeUrl:L}),d=u.modules.find(c=>c.uri===u.entryUri);return d?{ok:!0,js:s,runnableJs:d.code,runnableEntryUri:u.entryUri,runnableModules:u.modules,hasMain:$(o),diagnostics:a,entryUri:e.entryUri,graphEdges:u.modules.reduce((c,_)=>c+_.sourceImports.length,0),graphNodes:u.modules.length}:{ok:!1,js:"",runnableJs:"",runnableEntryUri:null,runnableModules:[],hasMain:!1,diagnostics:[{severity:"error",message:`No runnable entry module produced for ${e.entryUri}`,fileUri:e.entryUri}],entryUri:e.entryUri,graphEdges:0,graphNodes:0}}catch(n){const t=n instanceof Error?n.message:String(n);return{ok:!1,js:"",runnableJs:"",runnableEntryUri:null,runnableModules:[],hasMain:!1,diagnostics:[{severity:"error",message:t}],entryUri:e.entryUri,graphEdges:0,graphNodes:0}}},f=globalThis;f.compileLuminaProject=W;f.formatLuminaSource=F;export{W as compileLuminaProject,F as formatLuminaSource};
