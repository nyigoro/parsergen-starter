...
function highlightSnippet(input, location, useColor = true) {
  ...
  const lineStr = useColor ? prefix + chalk.redBright(targetLine) : prefix + targetLine;
  const pointerStr = useColor ? chalk.yellow(pointerLine) : pointerLine;
  const resultLines = [];
...
function formatErrorWithColors(error, useColors = true) {
  ...
  const errorMessage = error.error || "Unknown error";
  const parts = [
    `${chalk2.red("\u274C Parse Error:")} ${errorMessage}`
  ];
  if (error.location) {
    parts.push(`${chalk2.blue("\u21AA at")} ${formatLocation(error.location)}`);
  }
  if (error.expected && error.expected.length > 0) {
    parts.push(`${chalk2.yellow("Expected:")} ${error.expected.join(", ")}`);
  }
  if (error.found !== void 0) {
    parts.push(`${chalk2.yellow("Found:")} "${error.found}"`);
  }
  if (error.snippet || error.input && error.location) {
    try {
      ...
      parts.push("\n" + chalk2.dim("--- Snippet unavailable ---"));
    }
  }
  return parts.join("\n");
}
...
function formatErrorWithSuggestions(error, useColors = true) {
  ...
  const suggestionHeader = useColors ? chalk2.cyan("\n\u{1F4A1} Suggestions:") : "\n\u{1F4A1} Suggestions:";
  const formattedSuggestions = suggestions.map((suggestion, index) => {
    const bullet = useColors ? chalk2.dim(`  ${index + 1}.`) : `  ${index + 1}.`;
...
function extractReferences(expression) {
  ...
  if (matches) {
    const keywords = /* @__PURE__ */ new Set(["return", "if", "else", "while", "for", "function", "var", "let", "const"]);
...
// src/bin/cli.ts
var VALID_FORMATS = ["bare", "commonjs", "es", "globals", "umd"];
function printHelp() {
  console.log(`
