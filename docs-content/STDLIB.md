...
### forList(items: Signal<Vec<T>>, keyOf: fn(T, Int) -> String | Int, renderItem: fn(Signal<T>, Signal<Int>) -> VNode) -> VNode
...
### props_key(key: String | Int) -> Any

Marks a VNode as keyed. Keys must be unique among siblings. Keyed SSR emits a
`data-lumina-key` marker so `hydrate` and `hydrate_reactive` can adopt existing
DOM by identity.
...
Use `vec.get(index)` when the index may be missing. Use bracket indexing
(`vec[index]`) only when the index must exist; it returns `T` directly and
raises an out-of-bounds error for invalid indexes.
